...
import { isHTMLElement } from "../../helpers/guards";
import { BiBlock, FaGripVertical } from "react-icons/all";
...
function useDraggableBlockMenu(editor: LexicalEditor, anchorElem: HTMLElement, isEditable: boolean): JSX.Element {
  ...
  return createPortal(
    <>
      <button
        className="draggable-block-menu hover:bg-neutral-300 flex items-center justify-center rounded active:cursor-grabbing px-2
        py-1 cursor-grab opacity-0 absolute left-0 top-0 will-change-transform"
        ref={menuRef}
        draggable={true}
        onDragStart={onDragStart}
        onDragEnd={onDragEnd}>
        <FaGripVertical className={isEditable ? "inline-block w-[16px] h-[16px] pointer-events-none" : "hidden"} />
      </button>
      <div
        className="draggable-block-target-line pointer-events-none bg-sky-400 h-[4px]
         absolute left-0 top-0 opacity-0 will-change-transform"
        ref={targetLineRef}
      />
    </>,
    anchorElem,
